...
class beautifilter extends Module
{
	...
	{
		$this->name = 'beautifilter';
		$this->tab = 'front_office_features';
		$this->version = 1.4;
		$this->author = 'Vietmoonlight';

		parent::__construct();

		$this->displayName = $this->l('Beautiful product filter');
		$this->description = $this->l('Beautiful product filter using ajax.');
	}

	public function installHook(){
		$hook = Db::getInstance()->getRow('SELECT name FROM `'._DB_PREFIX_.'hook` WHERE `name`='.pSQL('beaufilter'));
		if(empty($hook)){
		Db::getInstance()->Execute('INSERT INTO `'._DB_PREFIX_.'hook` (`name`,`title`,`description`) ' .
		'VALUES ("beaufilter","beaufilter","Hook for Beaufilter module")');
		}
	}
	public function install()
	{
 		$this->installHook();
		if ($result = parent::install() AND $this->registerHook('beaufilter') AND $this->registerHook('header')
	...
	public function hookbeaufilter($params)
	{
	...
	{
		return $this->hookbeaufilter($params);
	}

	public function hookHeader($params)
	{
		Tools::addJS(($this->_path).'beautifilter.js');
		Tools::addJS(_PS_JS_DIR_.'jquery/jquery-ui-1.8.10.custom.min.js');
		Tools::addCSS(($this->_path).'beautifilter.css', 'all');
	...
	public function generateFiltersBlock($selectedFilters = array())
	{
		...
		return $smarty->fetch(_PS_MODULE_DIR_.$this->name.'/beautifilter.tpl');
	}
